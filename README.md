...
然后将目录放置到 `C:/Users/<用户名>/.codex/skills/paper-deep-reading-docx` 或 `~/.codex/skills/paper-deep-reading-docx`。
...
示例：使用 `$paper-deep-reading-docx`，按文件名前的三位编号精读论文，为每篇生成中文 Word 文档，并保留原文页码、图表和证据位置。
...
Copy the complete repository into your Codex skills directory. Do not copy only `SKILL.md`: `agents/`, `assets/`, `references/`, and `scripts/` are part of the skill.
...
Example: use `$paper-deep-reading-docx` to read papers in three-digit filename order, create one Chinese Word document per paper, and retain source pages, figures, tables, and evidence locations.
